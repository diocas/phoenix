...
Then('it should not be possible to share file/folder {string} using the webUI', async function (resource) {
  const state = await client.page.FilesPageElement.filesList().isSharingBtnPresent(resource)
  assert.ok(!state, `Error: Sharing button for resource ${resource} is not in disabled state`)
  const sidebarLinkTabState = await client.page.FilesPageElement.filesList().isSidebarLinksTabPresent(resource)
  assert.ok(!sidebarLinkTabState, `Error: Sidebar 'Links' tab for resource ${resource} is present`)
  const sidebarCollaboratorsTabState = await client.page.FilesPageElement.filesList().isSidebarCollaboratorsTabPresent(resource)
  assert.ok(!sidebarCollaboratorsTabState, `Error: Sidebar 'Collaborators' tab for resource ${resource} is present`)
})
