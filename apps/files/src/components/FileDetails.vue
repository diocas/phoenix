  ...
  <oc-app-side-bar id="files-sidebar" :key="highlightedFile.id" class="uk-padding-small uk-overflow-auto uk-height-1-1" :disableAction="false" @close="close()">
    ...
    <template slot="content">
      <div>
        ...
        <component v-if="fileSideBars.length > 0 && activeTabComponent" v-bind:is="activeTabComponent.component" @reload="$emit('reload')" :componentName="activeTabComponent.propsData.componentName" :componentUrl="activeTabComponent.propsData.componentUrl"></component>
      </div>
    </template>
  </oc-app-side-bar>
</template>
